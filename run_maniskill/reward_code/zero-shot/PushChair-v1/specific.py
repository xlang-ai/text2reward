...
def compute_dense_reward(self, action) -> float:
    ...
    gripper_coords = self.agent.get_ee_coords()
    ...
    chair_to_target_dist = np.linalg.norm(self.root_link.pose.p[:2] - self.target_xy)
    ...
    reward_vel = np.dot(chair_vel, (self.target_xy - self.root_link.pose.p[:2])) / (np.linalg.norm(chair_vel) * chair_to_target_dist)
